#define MODULE_NAME id3tag
#define MODULE_PATH id3tag
#define MODULE_STRING "id3tag"
#define __BUILTIN__
/*****************************************************************************
...
#define GET_STRING(frame,fidx) id3_ucs4_latin1duplicate( id3_field_getstring( &(frame)->fields[fidx] ) )
    ...
    {
        const char *psz_owner =
        
#ifdef MHSVLC
        (const char *)
#endif
        
        id3_field_getlatin1( &p_frame->fields[0] );
    ...
    for( i = 0; (p_frame = id3_tag_findframe( p_id3_tag, "TXXX", i )) != NULL; i++ )
    {
        ...
        (char *)
#endif
        GET_STRING( p_frame, 1 );
        char *psz_value = 
#ifdef MHSVLC
        (char *)
#endif
        
        GET_STRING( p_frame, 2 );
