...
#define MODULE_PATH access_file
...
# warning File size not known!
  ...
  /*  pk2=mdate();
       gap=pk2-pk1;
       pk=fopen("c:\\data\\sounds\\debug.txt","ab");
       if(gap>0)
           	{
           	fprintf(pk,"took long..");
           	}
       fprintf(pk,"Print took %d micros\r\n",gap);
       fclose(pk);*/
    return VLC_SUCCESS;
}
...
#else
    int fd = open (path, O_RDONLY | O_NONBLOCK /* O_LARGEFILE*/, 0666);
    if (fd == -1)
    {
