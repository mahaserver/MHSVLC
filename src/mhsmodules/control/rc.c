...
#define MODULE_PATH rc
#define MODULE_STRING "rc"
#define __BUILTIN__
/*****************************************************************************
...
static int Activate( vlc_object_t *p_this )
{
    ...
    msg_rc( _("MHSVLC CONTROL PANEL 1.0.0\nBy:Team MHSVLC\nThis version plays only Constant Bitrate MP3s\nType 'help' command for Help\n") );
    return VLC_SUCCESS;
}
...
static void Run( intf_thread_t *p_intf )
{
    ...
    while( !intf_ShouldDie( p_intf ) )
    {
        ...
        else if( var_Type( p_intf, psz_cmd ) & VLC_VAR_ISCOMMAND )
        {
            ...
            int i_ret;
#ifdef MHSVLC
            if(strcmp(psz_cmd,"add")==0){
            if(strcmp(psz_arg,"")==0){
            psz_arg="c:\\private\\200272B7\\mono.mp3";
            }
            }
#endif
